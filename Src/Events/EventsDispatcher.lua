...
local logger = Logger("Events")

local EventsDispatcher = {}
...
function EventsDispatcher.registerCallback(event, callback, handlerID)
	...
	if not callbackRegistry[event] then
	...
	if handlerID ~= nil then
		...
		handlerID = Strings.generateUniqueID(callbackRegistry[event])
	end
	callbackRegistry[event][handlerID] = callback
...
function EventsDispatcher.unregisterCallback(handlerID)
	...
	for eventName, eventRegistry in pairs(callbackRegistry) do
...
function EventsDispatcher.triggerEvent(event, ...)
	Assertions.isType(event, "string", "event")
	local registry = callbackRegistry[event]
	if registry then
...
function EventsDispatcher.hasCallbacksForEvent(event)
	Assertions.isType(event, "string", "event")
	return not Tables.isEmpty(callbackRegistry[event] or {})
end
