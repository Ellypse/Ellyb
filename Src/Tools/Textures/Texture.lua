...
--- @class Ellyb_Texture: MiddleClass
...
function Texture:GenerateString(width, height)
	...
	if self.coordinates then
		...
		return UiEscapeSequences.TEXTURE_WITH_COORDINATES:format(self:GetResource(), width, height, atlasWidth, atlasHeight, pxLeft, pxRight, pxTop, pxBottom)
	else
		return UiEscapeSequences.TEXTURE:format(self:GetResource(), width, height)
	end
end
